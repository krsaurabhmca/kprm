 ...
 * Shows all activities but only allowed tasks
 */
...
// Get allowed tasks for this TL
$allowed_tasks = get_user_allowed_tasks($user_id, 'TL');
$task_filter = '';
if (!empty($allowed_tasks)) {
    $task_ids_str = implode(',', array_map('intval', $allowed_tasks));
    $task_filter = " AND ct.task_template_id IN ($task_ids_str)";
} else {
    $task_filter = " AND 1=0"; // No access
}
...
// Cases Statistics (all cases, but filtered tasks)
$cases_stats = [
    'total' => 0,
    'pending' => 0,
    'in_progress' => 0,
...
$cases_sql = "SELECT 
    ...
    FROM cases 
    WHERE status != 'DELETED' $date_where";
...
// Tasks Statistics (filtered by allowed tasks)
$tasks_stats = [
    'total' => 0,
    'pending' => 0,
    'in_progress' => 0,
...
if ($has_case_tasks && !empty($allowed_tasks)) {
    $tasks_sql = "SELECT 
        COUNT(*) as total,
        ...
        FROM case_tasks ct
        WHERE ct.status = 'ACTIVE' $task_filter $date_where_tasks";
...
// Get allowed tasks list
$my_tasks = [];
if (!empty($allowed_tasks)) {
    $tasks_ids_str = implode(',', array_map('intval', $allowed_tasks));
    $tasks_sql = "SELECT id, task_name FROM tasks WHERE id IN ($tasks_ids_str) AND status = 'ACTIVE' ORDER BY task_name";
    $tasks_res = mysqli_query($con, $tasks_sql);
    if ($tasks_res) {
...
// Pending Tasks (Last 5) - Filtered by allowed tasks
$pending_tasks = [];
if ($has_case_tasks && !empty($allowed_tasks)) {
    $pending_tasks_sql = "SELECT ct.*, c.application_no, cl.name as client_name 
        ...
        LEFT JOIN clients cl ON c.client_id = cl.id 
        WHERE ct.status = 'ACTIVE' $task_filter AND (ct.task_status = 'PENDING' OR ct.task_status IS NULL)
...
// Tasks requiring review (filtered by allowed tasks)
$review_tasks = [];
if ($has_case_tasks && !empty($allowed_tasks)) {
    $review_sql = "SELECT ct.*, c.application_no, cl.name as client_name 
        ...
        LEFT JOIN cases c ON ct.case_id = c.id 
        LEFT JOIN clients cl ON c.client_id = cl.id 
        WHERE ct.status = 'ACTIVE' $task_filter AND ct.task_status = 'VERIFICATION_COMPLETED'
...
for ($i = 29; $i >= 0; $i--) {
    ...
    if ($has_case_tasks && !empty($allowed_tasks)) {
        $daily_tasks_sql = "SELECT COUNT(*) as tasks_count
            FROM case_tasks ct
            WHERE ct.status = 'ACTIVE' $task_filter AND DATE(ct.created_at) = '$date'";
    ...
    <div class="container-fluid py-3">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h4 class="mb-1">
                    ...
                    <strong>Team Leader Dashboard</strong>
                </h4>
                <p class="text-muted small mb-0">Overview of all activities for your assigned tasks</p>
            </div>
        ...
        <?php if (empty($allowed_tasks)): ?>
            <div class="alert alert-warning">
                ...
                <strong>No tasks assigned.</strong> Please contact Administrator to assign tasks to your account.
            </div>
        <?php else: ?>
        ...
        <div class="card mb-4">
            <div class="card-header bg-primary text-white">
                <h5 class="mb-0"><i class="fas fa-tasks me-2"></i>My Tasks (<?= count($my_tasks) ?>)</h5>
            </div>
            <div class="card-body">
                <div class="row">
                    <?php foreach ($my_tasks as $task): ?>
                        <div class="col-md-3 mb-3">
                            <div class="card border-start border-primary border-4">
                                <div class="card-body">
                                    <h6 class="mb-0"><?= htmlspecialchars($task['task_name']) ?></h6>
                                </div>
                            </div>
                        </div>
                    <?php endforeach; ?>
