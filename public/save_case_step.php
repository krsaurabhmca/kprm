...
if ($action == 'auto_save_client_meta') {
    // Auto-save client meta (AJAX only)
    ...
    if ($case_id > 0) {
        ...
        if ($case_result['status'] == 'success') {
            ...
            echo json_encode([
                'success' => false,
                'message' => 'Failed to auto-save: ' . ($case_result['message'] ?? 'Unknown error')
            ]);
        }
    } else {
        // Create new case
...
} elseif ($action == 'save_client_meta') {
    ...
    if ($case_task_result['status'] == 'success') {
        // If task type is 'iter', 'ITR' or 'ITO', send data using send_itr function
        $type_upper = strtoupper($task_template_data['task_type']);
        if ($type_upper == 'ITER' || $type_upper == 'ITR' || $type_upper == 'ITO') {
            $api_data = [
                "ref_id" => $case_task_result['id'],
                "name_of_document" => $task_template_data['task_name'] ?? 'ITR',
                "pan" => $task_meta['document_no'] ?? $task_meta['pan_no'] ?? '',
                "ay" => $task_meta['financial_year'] ?? '',
                "requirement" => $task_meta['requirement'] ?? "Added from KPRM"
            ];
            send_itr($api_data);
        }

        // If task type is 'BANKING', send data using send_banking function
        if ($type_upper == 'BANKING') {
            $api_data = [
                "ref_id" => $case_task_result['id'],
                "bank_name" => $task_meta['bank_name'] ?? '',
                "account_no" => $task_meta['account_no'] ?? '',
                "txn_details" => $task_meta['txn_details'] ?? $task_meta['transaction'] ?? $task_meta['transction'] ?? '',
                "requirement" => $task_meta['requirement'] ?? "Added from KPRM"
            ];
            send_banking($api_data);
        }
    }
    ...
    if ($case_task_result['status'] == 'success') {
        ...
        // If task type is 'iter', 'ITR' or 'ITO', send data using send_itr function
        $type_upper = strtoupper($task_type);
        if ($type_upper == 'ITER' || $type_upper == 'ITR' || $type_upper == 'ITO') {
            $api_data = [
                "ref_id" => $case_task_result['id'],
                "name_of_document" => $task_template_data['task_name'] ?? 'ITR',
        ...
        if ($is_ajax) {
            ...
            echo json_encode([
                ...
                'status' => 'success',
                'message' => $success_msg,
                'case_task_id' => $case_task_result['id'],
                'task_type' => $task_type,
                'task_name' => $task_template_data['task_name']
            ]);
            exit;
